...
type Config struct {
	...
	HashDB    database.HashBackendFactory // Configs for hash-based scheme
	PathDB    database.PathBackendFactory // Configs for experimental path-based scheme
}
...
type Database struct {
	...
	backend   database.Backend // The backend for managing trie nodes
}
...
func NewDatabase(diskdb ethdb.Database, config *Config) *Database {
	...
	if config.PathDB != nil {
		...
		db.backend = config.HashDB.New(diskdb, mptResolver{})
	} else {
		db.backend = HashDefaults.HashDB.New(diskdb, mptResolver{})
	}
	return db
}
